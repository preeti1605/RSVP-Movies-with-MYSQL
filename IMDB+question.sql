...
SELECT 
		SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS no_of_null_id, 
		SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS no_of_null_title, 
		SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS no_of_null_year,
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS no_of_null_date_published,
		SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS no_of_null_duration,
		SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS no_of_null_country,
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS no_of_null_worlwide_gross_income,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS no_of_null_languages,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS no_of_null_production_company
...
SELECT MONTH(date_published) AS monthNum, COUNT(id) AS number_of_Movies 
FROM movie
...
FROM genre AS gen
INNER JOIN ratings AS rating
ON gen.movie_id = rating.movie_id
INNER JOIN movie AS mov
ON mov.id = gen.movie_id
WHERE mov.country='USA' AND rating.total_votes>1000 AND MONTH(date_published)=3 AND year=2017
GROUP BY gen.genre
ORDER BY movie_Count DESC;
...
ON dm.movie_id = gen.movie_id 
INNER JOIN ratings AS rating
ON rating.movie_id = gen.movie_id,
top_genre
WHERE gen.genre in (top_genre.genre) AND avg_rating>8
...
SELECT DISTINCT name AS actor_name, COUNT(rating.movie_id) AS movie_Count
FROM ratings AS rating
INNER JOIN role_mapping AS rm
ON rm.movie_id = rating.movie_id
